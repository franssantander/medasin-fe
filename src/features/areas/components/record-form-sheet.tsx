    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full overflow-y-auto sm:max-w-md">
        <SheetHeader>
          ...
          <SheetDescription>
            Keep this information connected to the current area.
          </SheetDescription>
        </SheetHeader>
        <form id="record-form" onSubmit={submit} className="grid gap-4 px-4">
          ...
          <label className="flex items-center gap-2 text-sm">
            <input
              type="checkbox"
              checked={Boolean(
                form[kind === "habit" ? "is_active" : "is_pinned"],
              )}
              onChange={(e) =>
                change(
                  kind === "habit" ? "is_active" : "is_pinned",
                  e.target.checked,
                )
              }
            />
            {kind === "habit" ? "Active" : "Pin this note"}
          </label>
          {error && <p className="text-sm text-destructive">{error}</p>}
        </form>
        <SheetFooter>
          <Button form="record-form" type="submit" disabled={isPending}>
