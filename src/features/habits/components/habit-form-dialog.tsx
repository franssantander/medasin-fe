...
import { habitSchema, type HabitFormValues, type HabitResolvedValues } from "../schemas/habit-schema";
    ...
    >
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Define a repeatable action and decide where it belongs.
          </DialogDescription>
        </DialogHeader>
        <form id="global-habit-form" onSubmit={submit} className="grid gap-5">
          <FormField label="Habit name" error={errors.name?.message}>
            <Input {...register("name")} autoFocus placeholder="e.g. Read for 20 minutes" />
          </FormField>
          <FormField label="Icon" error={errors.icon?.message}>
            <div className="overflow-hidden rounded-xl border">
              <div className="flex items-center gap-3 border-b p-3">
                <div className="flex size-8 shrink-0 items-center justify-center rounded-md bg-primary text-primary-foreground">
                  <AreaIcon name={selectedIcon || "Repeat2"} className="size-3.5" />
                </div>
              ...
              <div className="grid max-h-48 grid-cols-[repeat(auto-fill,2rem)] justify-between gap-1 overflow-y-auto p-3">
                {filteredIcons.map(({ name, icon: Icon }) => (
                  <button
                    ...
                    onClick={() => setValue("icon", name, { shouldDirty: true, shouldValidate: true })}
                  >
          ...
          <FormField label="Description" error={errors.description?.message}>
            <Textarea {...register("description")} className="min-h-20" placeholder="Why does this habit matter?" />
          </FormField>
            ...
            <Controller
              ...
              render={({ field }) => (
                <Select value={field.value} onValueChange={(value) => value && field.onChange(value)}>
                  ...
                  <SelectContent align="start">
                    {frequencies.map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}
                  </SelectContent>
                </Select>
              )}
            />
          ...
          {(frequency === "weekly" || frequency === "custom") && (
            <FormField label="Days of the week" error={errors.schedule_days?.message}>
              <div className="grid grid-cols-7 gap-2">
                {weekdays.map((day) => (
                  <Button key={day.value} type="button" size="sm" variant={selectedDays.includes(day.value) ? "default" : "outline"} className="px-0" onClick={() => toggleDay(day.value)}>
                    {day.short}
                  </Button>
                ))}
              </div>
          ...
          {frequency === "monthly" && (
            <FormField label="Dates of the month" error={errors.schedule_dates?.message}>
              <div className="grid grid-cols-7 gap-1.5">
                {Array.from({ length: 31 }, (_, index) => index + 1).map((date) => (
                  <button key={date} type="button" className={cn("h-8 rounded-md border text-xs transition-colors", selectedDates.includes(date) ? "border-primary bg-primary text-primary-foreground" : "hover:bg-accent")} onClick={() => toggleDate(date)}>
                    {date}
                  </button>
                ))}
              </div>
            ...
            render={({ field }) => (
              <label className="flex items-center gap-3 rounded-lg border p-3 text-sm">
                ...
                <span>
                  ...
                  <span className="text-muted-foreground">Paused habits keep their history and stop requesting check-ins.</span>
                </span>
              </label>
            )}
          />
